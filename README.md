...
├── packaging/
│   ├── gqfilter_macos.spec     # PyInstaller設定（macOS）
│   ├── gqfilter_linux.spec     # PyInstaller設定（Linux）
│   ├── gqfilter.spec           # PyInstaller設定（Windows）
│   └── gqfilter.wxs            # WiX MSI定義ファイル
...
```bash
# macOS / Linux
./dist/gqfilter/gqfilter -i input.las -o output.las [オプション...]

# Windows
dist\gqfilter\gqfilter.exe -i input.las -o output.las [オプション...]
```
...
# 出力: dist/gqfilter/gqfilter
# アーカイブ: dist/gqfilter_macos.zip
```
...
# 出力: dist/gqfilter/gqfilter
# アーカイブ: dist/gqfilter_linux.tar.gz
```
