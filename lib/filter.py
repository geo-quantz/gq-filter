...
def get_origin_auto(path: str) -> Optional[Tuple[float, float, float]]:
    """
    pye57 を使用して、E57ファイルのヘッダーから直接器械点を抽出する（Cyclone対応版）
    """
    ...
    try:
        ...
        if hasattr(header, 'translation'):
            pos = header.translation
            tx, ty, tz = float(pos[0]), float(pos[1]), float(pos[2])
            print(f"[INFO] {os.path.basename(path)} の器械点座標を取得しました: X={tx:.3f}, Y={ty:.3f}, Z={tz:.3f}")
            return (tx, ty, tz)
        else:
            print(f"[WARNING] ヘッダーに translation 情報が見つかりません: {path}")
            return None
            
    except Exception as e:
        print(f"[ERROR] pye57での器械点取得に失敗しました: {e}")
        return None
...
def build_incidence_angle_filter(
    ...
    # sqrtやacosを使わない超高速な入射角（内積の2乗）計算
    # 距離の2乗 (dist_sq)
    dist_sq_expr = f"((X - {x0})*(X - {x0}) + (Y - {y0})*(Y - {y0}) + (Z - {z0})*(Z - {z0}))"
    # 内積 (dot) と 内積の2乗 (dot_sq)
    dot_expr = f"((X - {x0})*NormalX + (Y - {y0})*NormalY + (Z - {z0})*NormalZ)"
    dot_sq_expr = f"({dot_expr} * {dot_expr})"
...
def build_range_filter(params: Optional[RangeParams], input_path: str) -> Optional[Dict[str, Any]]:
    ...
    conds = []
    if params.min_distance is not None:
        min_sq = float(params.min_distance * params.min_distance)
        conds.append(f"({dist_sq_expr} >= {min_sq})")
        
    if params.max_distance is not None:
        max_sq = float(params.max_distance * params.max_distance)
        conds.append(f"({dist_sq_expr} <= {max_sq})")
...
#    return {"pipeline": stages}
    ...
    # 基本の出力設定
    writer_stage = {"filename": output_path}

    if ext_out == ".e57":
        writer_stage["type"] = "writers.e57"
    elif output_path.endswith(".copc.laz") or ext_out in [".las", ".laz"]:
        writer_stage["type"] = "writers.copc" if output_path.endswith(".copc.laz") else "writers.las"
        
        # ★ LAS/LAZ 出力時のスケールとオフセットを明示的に指定
        # スケールを 0.001（1ミリ単位）に設定し、座標の丸め込みを防ぐ
        writer_stage["scale_x"] = 0.001
        writer_stage["scale_y"] = 0.001
        writer_stage["scale_z"] = 0.001
        writer_stage["offset_x"] = "auto"
        writer_stage["offset_y"] = "auto"
        writer_stage["offset_z"] = "auto"
        
        # もし入力ファイルがLASだった場合、元のスケール設定を優先して引き継ぐ
        writer_stage["forward"] = "all"
        
    elif ext_out in [".txt", ".csv"]:
        writer_stage["type"] = "writers.text"
    else:
        writer_stage["type"] = "writers.las"
